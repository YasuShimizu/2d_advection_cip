...
f0=np.zeros([nx1,ny1])
fx=np.zeros([nx1,ny1]);fxn=np.zeros([nx1,ny1])
fy=np.zeros([nx1,ny1]);fyn=np.zeros([nx1,ny1])
...
for i in np.arange(0,nx):
    for j in np.arange(0,ny):
        ...
        if r<rb:
            ...
            f0[i,j]=0.
f=copy.copy(f0); fn=copy.copy(f)

# Initial fx, fy
fx,fy=initial.fxfy_init(f,fx,fy,nx,ny,dx,dy)
fxn=copy.copy(fx); fyn=copy.copy(fy)
...
#    print(time,theta,u,v)
    ...
    fn,fxn,fyn=cip2d.f_cal(f,fn,fx,fy,fxn,fyn,nx,ny,dx,dy,dt,u,v)
    f=copy.copy(fn)
    fx=copy.copy(fxn)
    fy=copy.copy(fyn)
    ...
    if (icount % fskip) == 0:
        ...
        ax.set_title('2D CIP Circular Cone',size='32')
